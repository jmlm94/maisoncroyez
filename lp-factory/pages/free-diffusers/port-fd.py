...
# 12. fd4 (2026-09-19): one-time = "(One-Time)" kit variants (80.00 / 90.05 / 140.10) + full-price scents => $129.95 / $189.95 / $289.95
for old, new in [('price: 49.95,  scents: 1,', 'price: 49.95,  oneTime: 80.00,  scents: 1,'), ('price: 99.90,  scents: 2,', 'price: 99.90,  oneTime: 90.05,  scents: 2,'), ('price: 149.85, scents: 3,', 'price: 149.85, oneTime: 140.10, scents: 3,'), ('  today() { return this.tier().price + this.extras() * this.scentPrice() + (this.oneTime() ? this.tier().n * DIFFUSER_PRICE : 0); },', '  today() { return this.tier().price + this.extras() * this.scentPrice() + (this.oneTime() ? this.tier().oneTime : 0); }, /* fd4: one-time = kit price + one-time kit variant (80.00 / 90.05 / 140.10) => $129.95 / $189.95 / $289.95 */'), ('  oneTimeDiffuser: 45784228429933,   /* Special Kits "1 Diffuser" $79.95, added x N on one-time orders */\n', '  oneTimeKits: { one: 45900920324205, two: 45900920356973, three: 45900920389741 },  /* same product, "(One-Time)" variants $80.00 / $90.05 / $140.10 (fd4, 2026-09-19) */\n'), ('  const items = sub ? [{ id: CART3.kitVariants[T.key], quantity: 1 }] : [];\n', '  const items = [{ id: sub ? CART3.kitVariants[T.key] : CART3.oneTimeKits[T.key], quantity: 1 }];\n'), ('  if (!sub) items.push({ id: CART3.oneTimeDiffuser, quantity: T.n });\n', ''), ('<span class="kr-pr">${sel.oneTime() ? usd(T.n * DIFFUSER_PRICE) : html`<s>${usd(T.n * DIFFUSER_PRICE)}</s><span class="inc">FREE</span>`}</span>', '<span class="kr-pr">${sel.oneTime() ? html`<s>${usd(T.n * DIFFUSER_PRICE)}</s> ${usd(T.oneTime)}` : html`<s>${usd(T.n * DIFFUSER_PRICE)}</s><span class="inc">FREE</span>`}</span>'), ('<span class="popt-line"><b>${usd(T.price + T.n * DIFFUSER_PRICE)} today.</b> Diffusers charged ${usd(DIFFUSER_PRICE)} each. No refills.</span>', '<span class="popt-line"><b>${usd(T.price + T.oneTime)} today.</b> ${T.n} scent${T.n > 1 ? "s" : ""} at full price + ${T.n === 1 ? "the diffuser" : T.n === 2 ? "both diffusers" : "all 3 diffusers"} for ${usd(T.oneTime)}. No refills.</span>')]:
