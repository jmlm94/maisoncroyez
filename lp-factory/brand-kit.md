...
- Live page #3: `/pages/diffuser` (gid://shopify/Page/117264089197, DEFAULT template — renders INSIDE theme layout per owner; body-embed loader, deployed 2026-07-08). Cold-traffic PDP, Blueprint 003. Real cart: diffuser 45216681590893 + chosen fragrance variant on Subi selling plan 1605206125 -> /cart.
  ...
  - Loader version key: `'p1-'+hour`
  - **PERF (2026-07-08): media DE-EMBEDDED.** The 23 media files now live as individual CDN files `mc-lp-d-*` (images = MediaImage, videos = GenericFile), referenced by URL in `mc-lp-diffuser-assets.js` (2.3KB, was 1.68MB base64). This is DEPLOY-ONLY; the preview artifact still base64-embeds via build.py for self-containment. Mobile Lighthouse was 18 (LCP 17.1s) pre-fix. Hero (gallery img[0]) preloaded + fetchpriority high; videos preload=none; FB pixel deferred to load+1.2s. To re-deploy assets, regenerate the URL map (see pages/pdp-diffuser/upload_all.py for the upload rail). (bump rN via pageUpdate on every fileUpdate). LESSON: Shopify file origin takes ~5 min to propagate after fileUpdate — bump rN, then WAIT 5 MIN before verifying, or the new key caches the old bytes at the edge.
