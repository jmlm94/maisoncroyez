...
const TIERS = [
  { key: "one",   n: 1, name: "1 FREE Diffuser + 1 Scent \uD83C\uDF81",  req: "",  price: 49.95,  oneTime: 80.00,  scents: 1, tag: "",             lite: true,  line: "For small spaces only: Restroom, Studio, Storage.", ship: "ELIGIBLE FOR FREE SHIPPING", shipFree: true, tags: [], grad: "linear-gradient(135deg,#FBEBDD 0%,#F6D9C4 100%)" },
  { key: "two",   n: 2, name: "2 FREE Diffusers + 2 Scents \uD83C\uDF81", req: "", price: 99.90,  oneTime: 60.05,  scents: 2, tag: "MOST POPULAR", lineStrong: true, line: "Living Room + Bedroom. The two rooms you actually live in.", ship: "ELIGIBLE FOR FREE SHIPPING", shipFree: true, tags: [], grad: "linear-gradient(135deg,#FCE4EC 0%,#E9DDF7 100%)" },
  { key: "three", n: 3, name: "3 FREE Diffusers + 3 Scents \uD83C\uDF81", req: "", price: 149.85, oneTime: 90.10, scents: 3, tag: "BEST VALUE",   lineStrong: true, line: "Whole home: Living Room, Bedroom, Kitchen. Nothing left unscented.", ship: "ELIGIBLE FOR FREE SHIPPING", shipFree: true, tags: [], grad: "linear-gradient(135deg,#E4F3EA 0%,#D9ECF7 100%)" },
];
            ...
            ${CONFIG.fragrances.map((f) => { const q = sel.qty(f.key); const on = q > 0; const full = T.scents > 0 && sel.keys.length >= T.scents; return html`
              <div key=${f.key} class=${"pick compact" + (on ? " on" : "")}
                ...
                <span class="pick-foot" onClick=${(e) => e.stopPropagation()}>
                  <span class="pick-free"><span class="pf-price">${usd(SCENT_ONE)}</span> + FREE diffuser \uD83C\uDF81</span>
                  <span class="pick-qty">
