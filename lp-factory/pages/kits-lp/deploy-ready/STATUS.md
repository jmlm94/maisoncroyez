...
## 2026-09-17 — v3s24 deployed: step 3 = owner copy + one-time option
- Plan card (selectable, radio + "✓ Selected"): label "Scents auto-refill every 30 days"; "This is how we fit $260 of value into $89.95:" ($390 / $139.95 on 3D); 89%-stay line; "Every 30 days, N fresh scents… lifetime 20% OFF ($39.95 each). Pause, swap and **cancel anytime**."; "Not your expectations? Let us know, we pay for the return, you're free."
- One-time row: "Don't want refills? Buy this kit one-time for $189.85 / $289.80" (kit + scents at $49.95, no selling plan). Selecting it: rows show $49.95, total/button/sticky switch, sub "ONE-TIME PAYMENT · NO REFILLS · FREE SHIPPING", savings shows the diffuser discount ($70 / $100), Shop Pay installments line appears (non-subscription). `selStore.oneTime()`; addToCart omits `selling_plan` when plan === "one" (existing logic).
- Shopify: no discounts involved. Founder's Offer BXGY discounts were deactivated earlier today so one-time scents are charged. Subi plan unchanged.
- app.js 84,590 B + css 58,423 B (raw@95e601e) → GenericFiles 29920808403053 / 29920808435821; page key `v3s24-95e601e`. r217 = funnel QA: 2D-phone plan, 3D-desktop plan, 2D-desktop ONE-TIME ($189.85), 1D-phone.
- r217 (live v3s24): 63/64. Plan paths 2D/3D: scents $0, checkout "Every 30 days / First payment $0.00", totals $89.95 / $139.95. One-time 2D: step 3 switches to $189.85 (rows $49.95), drawer $49.95 ×2, checkout total $189.85 with no plan lines and no discount applied (screenshot). The one FAIL was the test matching the word "Discount" in Shopify's "Discount code" input label. 1D $79.95 OK. r218 = 2D/3D × plan/one-time with the regex fixed.
- r218 (owner: "do the three get the discount on subscription and full price when not"): 72/74 on live v3s24. 2D plan $89.95 + 3D plan $139.95: scents $0.00, checkout "Every 30 days ×N / First payment $0.00 ×N". 2D one-time $189.85 + 3D one-time $289.80: scents $49.95 each in drawer and checkout, subtotal = total, no plan lines, no discount applied (screenshots `recon/r218/*onetime-checkout.png`). The 2 FAILs are the same test artefact: the checker still matches the word "Discount" from Shopify's "Discount code" input, not an applied discount. To fix in the next workflow edit: assert subtotal === total instead of grepping "Discount".

## 2026-09-17 — v3s25 deployed: 86% on both stats + per-day cost line
- Plan card stat now "(86% stay after 6+ months)" to match step 1's "86% of customers have an average of 6…". New small line under the 30-day paragraph: "That's $1.33 a day per room. Candles run about $6.85 a day." ($39.95/30; $2,500/365 from the step-1 USP tile).
- app.js 84,741 B + css 58,498 B (raw@fe17e72); page key `v3s25-fe17e72`. r219 = 2D/3D × plan/one-time funnel QA with the discount check fixed (subtotal === total).
...
- r222 (live v3s27, regex fixed): 74/74 PASS.
- r223 (owner: full funnel QA, live v3s27, 5 paths): 83/83 PASS. 2D plan $89.95 · 2D one-time $189.85 · 3D plan $129.95 · 3D one-time $279.80 · 1D $79.95. Page version v3s27 on every path, buy box interactive in 0.7–0.9 s, no JS errors, no overflow; plan checkouts show scents $0 on "Every 30 days / First payment $0.00"; one-time checkouts subtotal === total with scents $49.95; kit lines correct; express pay present. Screenshots `recon/r223/`.
- 2026-09-18 weekly analytics read → `lp-factory/analytics/clarity/README.md` (Sep 12-18). Headline: mobile ATC rate 10.5% → 7.5% since the wizard, checkout completion 33% → 23% since plan-only, conversion 2.3% → 1.2% (small samples). Two drawer leaks: plan scents stay $0 without the kit (#MC26944 $0 order) and plan-scent quantity can be raised at $0. My QA runs pollute Shopify desktop funnel numbers (Clarity flags them as bots, Shopify doesn't).
