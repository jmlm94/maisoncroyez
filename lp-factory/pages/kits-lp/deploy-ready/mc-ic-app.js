...
const CONFIG = {
  ...
  announcement: {
    ...
    text: "Get 2 scents and the diffuser is 100% on us.",
    cta: "",
  },
  ...
  buybox: {
    ...
    accordions: [
      { q: "Do you offer free shipping?", a: "Yes \u2014 every order ships free, always. No minimums, no surprises." },
      { q: "How do I cancel or pause?", a: "Two clicks from your account or any email we send. Skip a delivery, pause, swap scents, or cancel \u2014 no calls, no fees, no questions." },
      ...
      { q: "What about warranty and guarantees?", a: "Cancel any time in your first 90 days and we refund every dollar \u2014 prepaid return label on us. After 90 days, everything is yours to keep. Plus a Lifetime Warranty on every diffuser." },
    ],
  },
  ...
  guarantee: {
    ...
    bullets: [
      ...
      "Cancel any time in your first 90 days and **we refund every dollar** — prepaid return label on us. After 90 days? **Everything is yours to keep.**",
      "And the diffuser itself? **Covered by a Lifetime Warranty.**",
    ],
    cta: { label: "Claim my free diffuser", sub: "2 scents · $89.95 every 45 days · cancel in 2 clicks" },
  },

  faq: {
    heading: ["Questions?", "We've got answers."],
    items: [
      { q: "When am I charged?", a: "Once today ($89.95), then $89.95 every 45 days when your next 2 scents ship. You get a heads-up before every charge — skip or pause in one tap and you're not charged. No hidden fees, ever." },
      { q: "How do I cancel?", a: "Two clicks — from your account or any email we send. No phone calls, no chat queues, no retention offers. Cancel in your first 90 days and we email a prepaid return label for a full refund; after 90 days, everything is yours to keep." },
      { q: "Can I be charged without knowing?", a: "No. Every charge is announced ahead of time, with skip and pause one tap away. If you skip, nothing ships and nothing is charged." },
      { q: "Is the diffuser really free?", a: "Yes. As an Inner Circle member you pay only $89.95 every 45 days for your 2 scents — the diffuser itself is $0, and after 90 days it's yours to keep no matter what." },
      ...
      { q: "What if I don't love it?", a: "Live with it for 90 days. If your home doesn't feel different, cancel — we send a prepaid label and refund every dollar. And every diffuser carries a Lifetime Warranty." },
    ],
  },
...
async function addToCart(setBusy, setToast) {
  const mode = selStore.mode;
  const left = mode === "sub" ? selStore.count - selStore.keys.length : 0;
  if (left > 0) {
    setToast("Pick " + left + " more scent" + (left > 1 ? "s" : "") + " to start your membership.");
    return;
  }
  if (!onStore()) {
    setToast(mode === "sub"
      ? "Preview mode. On the live store this adds your 2 scents on the 45-day plan plus your free diffuser, and opens the cart drawer."
  ...
  const trackValue = mode === "sub" ? 89.95 : 89.95 + 49.95 * selStore.keys.length;
  try { if (window.fbq) fbq("track", "AddToCart", { content_type: "product", content_ids: items.map((i) => String(i.id)), value: trackValue, currency: "USD", num_items: items.length }); } catch (e) {}
  try {
    setBusy(true);
...
const selStore = {
  count: 2,
  kitIdx: 1,
  mode: "sub",                        /* free-diffuser offer: membership pre-selected */
  scarce: 39,                         /* today's free-diffuser counter (mock) */
  setMode(m) { this.mode = m; this.count = m === "sub" ? 2 : 7; this.keys = m === "sub" ? ["love", "abundance"] : []; this.emit(); },
  plan: "renew",                      /* Renew & Save pre-selected (spec 03) */
  keys: ["love", "abundance"],        /* pre-picked bestsellers — swap any */
  listeners: new Set(),
...
if (!window.__mcScarceTick) { window.__mcScarceTick = setInterval(() => { if (selStore.scarce > 17) { selStore.scarce -= 1; selStore.emit(); } }, 30000); }
    ...
    <section class="section pdp-buy" id="buybox">
      <div class="wrap">
        ...
        <div class="buybox">
          ...
          <div class="picker-title plansel-title">1. How would you like to transform your spaces?</div>
          <div class="offwrap">
            <div class=${"offhero" + (sel.mode === "one" ? " dim" : "")} onClick=${() => sel.setMode("sub")} role="button" tabindex="0"
              onKeyDown=${(e) => { if (e.key === "Enter" || e.key === " ") sel.setMode("sub"); }}>
              <span class="kg-tag off-stat">87% of members stay for 6+ months, cancel anytime.</span>
              <span class="off-title">Inner Circle Membership ✨</span>
              <span class="off-hook">Select two (2) scents and the diffuser is 100% on us.</span>
              <span class="off-ledger">
                <span class="off-lrow"><span class="off-llabel">THE DIFFUSER</span><span class="off-lval"><s class="off-was">$89.95</s> <b class="off-price">FREE</b></span></span>
                <span class="off-lrow"><span class="off-llabel">YOUR 2 SCENTS</span><span class="off-lval off-lcol"><span><b class="off-scent-price">$89.95</b> today</span><span class="off-lv2">Renews every 45 days</span></span></span>
              </span>
              <span class="off-val">$189.85 of product — Yours for $89.95 today!</span>
              <span class="off-line">Pause, Skip, Swap anytime.</span>
              <span class="off-plus">PLUS:</span>
              <span class="off-badges"><span class="off-badge">🚚 FREE SHIPPING</span><span class="off-badge">💎 20% OFF NEW SCENTS — FOREVER</span><span class="off-badge">⏸️ CANCEL ANYTIME</span></span>
              <span class="off-scarce">Today’s free diffusers: <b>${sel.scarce} of 100 left</b></span>
              <span class="whyfree"><b>Why give the diffuser away?</b> Because once your home runs on scent for 45 days, you won’t want to stop. We’d rather earn you every delivery than charge you at the door.</span>
            </div>
            <span class="optout">${sel.mode === "sub"
              ? html`No thanks — <a href="#" onClick=${(e) => { e.preventDefault(); sel.setMode("one"); }}>I’ll just buy the diffuser for $89.95</a> <span class="optout-sub">(Scents for $49.95/each)</span>`
              : html`<b>You’re buying the diffuser only — $89.95 + shipping.</b> <a href="#" onClick=${(e) => { e.preventDefault(); sel.setMode("sub"); }}>← Back to the free-diffuser offer</a>`}</span>
          </div>

          <div class="picker-title">${sel.mode === "sub" ? B.pickerTitle : "2. Add scents to your order — for $49.95 each (Optional):"}</div>
          ...
          <div class="pick-count">${sel.mode === "sub" ? sel.keys.length + "/2 selected" + (sel.complete() ? " \u2713" : "") : "Optional add-ons \u2014 " + sel.keys.length + " added"}</div>
              ...
              <div key=${f.key} class=${"pick" + (on ? " on" : "")}
                ...
                <span class="pick-row">
                  ...
                  <span class="pick-txt">
                    ...
                    <span class="pick-free">${sel.mode === "sub" ? html`<s>$49.95</s> Included!` : html`$49.95 each`}</span>
                  </span>
                  ${sel.count > 1
          ...
          <div class="howworks">
            <h3><span class="hw-emoji">💡</span>This is the way your membership works:</h3>
            <div class="hw-step"><span class="hw-ico">🎁</span><span><b>Your diffuser is FREE.</b> You only pay $89.95 every 45 days (less charges) that covers your 2 scents, shipped free.</span></div>
            <div class="hw-step"><span class="hw-ico">⏸️</span><span><b>Skip or pause whenever you want.</b> Not ready for your next delivery? One click moves it, straight from your account — nothing ever surprises you.</span></div>
            <div class="hw-step"><span class="hw-ico">✌️</span><span><b>Need to cancel? This is how it works:</b> Cancel in your first 90 days and we email you a prepaid return label; send everything back and you’re done. After 90 days? Just cancel — everything is yours to keep, nothing to return, ever.</span></div>
            <div class="hw-step"><span class="hw-ico">🧾</span><span><b>A heads-up about checkout:</b> you’ll see a “subscription” line when you pay. That’s just the formality that powers your deliveries — everything written above is the real deal.</span></div>
            <div class="hw-dash-t">This is how your dashboard looks like:</div>
            <div class="portal">
              <div class="portal-top"><span class="portal-brand">MAISON CROYEZ</span><span>My membership</span></div>
              <div class="portal-next">Next delivery: ${new Date(Date.now() + 45 * 864e5).toLocaleDateString("en-US", { month: "long", day: "numeric" })}</div>
              <div class="portal-sub">2 scents: $89.95 + Free Shipping</div>
              <div class="portal-btns"><span class="portal-btn">Skip this delivery</span><span class="portal-btn">Pause</span><span class="portal-btn">Swap scents</span><span class="portal-btn danger">Cancel membership</span></div>
              <div class="portal-cap">This exact screen is in your account from day one. Every button works instantly — no calls, no chat queues, no one to convince. Your membership answers to you, always.</div>
            </div>
            <div class="tl">
              <div class="tl-step"><b>Today</b>Diffuser & 2 scents ship.</div>
              <div class="tl-step"><b>Day 38</b>Almost out, heads up before delivery</div>
              <div class="tl-step"><b>Day 45</b>Your 2 scents arrive at your doorstep</div>
            </div>
          </div>
...
function StickyBar() {
  ...
  const left = sel.mode === "sub" ? sel.count - sel.keys.length : 0;
  ...
  return html`
    <div class=${"sticky" + (show ? " show" : "")}>
      <button class="btn" disabled=${busy} onClick=${() => incomplete ? goPick() : addToCart(setBusy, setToast)}>
        <span>${busy ? "Adding\u2026" : incomplete ? "Choose your scents \u2794" : (sel.mode === "sub" ? "CLAIM MY FREE DIFFUSER \u2794" : "ADD TO CART \u2014 $89.95 \u2794")}</span>
        <span class="btn-sub">${incomplete ? left + " scent" + (left > 1 ? "s" : "") + " left \u2014 then add to cart" : (sel.mode === "sub" ? "$89.95 today \u2014 2 scents + free diffuser + free shipping" : "One-time \u2014 scents $49.95 each")}</span>
      </button>
      <${Toast} msg=${toast} onClose=${() => setToast("")}/>
    </div>`;
}
