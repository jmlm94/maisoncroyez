...
const KITS = [
  ...
  { key: "house", name: "House Kit", price: 339.95, value: 439.95, variant: 45644597002349, diffusers: 3, scents: 5,
    img: "https://cdn.shopify.com/s/files/1/0020/3636/7469/files/mc-kb-kit3.jpg?width=160",
    line: "Diffusers (3) + full-size scents (5) as our gift 🎁", days: "225+ days of FREE SCENT", per: "", tag: "" },
];
const OFFER = { price: 139.95 }; /* min kit price (legacy refs) */
...
const CONFIG = {
  ...
  announcement: {
    ...
    text: "Just for today: FREE priority shipping — a $19.95 value",
    cta: "",
  },
  ...
  buybox: {
    ...
    pickerLabel: "Tap to select. Repeats welcome — stock up on the one your home loves.",
    ...
    accordions: [
      ...
      { q: "What about warranty and guarantees?", a: "Two layers. First, a 30-day money-back guarantee: if your space doesn't feel different, we refund every dollar and send prepaid return labels. Second, every diffuser carries a Lifetime Warranty: malfunctions, leaks, anything — we replace it, forever." },
    ],
  },
  ...
  images: {
    ...
    frag1: { file: "frag1", src: T96(A.frag1) }, frag2: { file: "frag2", src: T96(A.frag2) },
    frag3: { file: "frag3", src: T96(A.frag3) }, frag4: { file: "frag4", src: T96(A.frag4) },
    frag5: { file: "frag5", src: T96(A.frag5) }, frag6: { file: "frag6", src: T96(A.frag6) },
    frag7: { file: "frag7", src: T96(A.frag7) },
...
async function addToCart(setBusy, setToast) {
  ...
  if (!onStore()) {
    setToast("Preview mode. On the live store this adds the " + kit.name + ": " + kit.diffusers + " diffuser" + (kit.diffusers > 1 ? "s" : "") + " + " + selStore.label() + " for " + usd(kit.price) + " one-time — no subscription — and opens the cart drawer.");
    return;
  }
...
const selStore = {
  count: 3,
  kitIdx: 1,
  plan: "onetime",                    /* kits are one-time purchases */
            ...
            ${CONFIG.fragrances.map((f) => { const q = sel.qty(f.key); const on = q > 0; return html`
              <div key=${f.key} class=${"pick" + (on ? " on" : "")}
                ...
                <span class="pick-row">
                  ...
                  <span class="pick-txt">
                    <span class="pick-name">${f.name}${f.key === "love" ? html` <span class="pick-love">❤️ #1 Bestseller</span>` : (f.topSeller ? " 🏆" : "")}</span>
...
function StickyBar() {
  ...
  return html`
    <div class=${"sticky" + (show ? " show" : "")}>
      <button class="btn" disabled=${busy} onClick=${() => addToCart(setBusy, setToast)}>
        <span>${busy ? "Adding…" : "ADD TO CART · " + usd(sel.kit().price) + " ➔"}</span>
        <span class="btn-sub">${sel.kit().name + " · " + (sel.label() || "pick your scents")}</span>
      </button>
      <${Toast} msg=${toast} onClose=${() => setToast("")}/>
    </div>`;
}
...
/* cart drawer: handled by the sitewide mc-drawer.js loader (theme.liquid). */
